...
//copy these function headers into a low level header file like groundwork.h
...
class bitmap
	...
	public:
		...
		BYTE get_pixel(int x, int y,int color_offset) //color_offset = 0,1 or 2 for blue, green ,red
			{
...
class level
	...
	public:
		...
		bool check_wall(float posx, float posy)
		{
			posx = -(posx) / 2 + 50.5;
			posy = -(posy) / 2 + 0.5;
			BYTE blue = leveldata.get_pixel(posx, posy, 0);
			if(blue == 0)
			{
				return true;
			}
	...
	class camera
		...
		public:
			...
			camera()
				{
				w = s = a = d = 0;
				position = position = XMFLOAT3(0, 0, -2);
				}
			void animation(bitmap *leveldata, level &alevel)//bitmap in the argument NOW YOU CAN USE IT HERE!!! *NEW*
			{
				XMMATRIX R, T;
				...
				float new_positionx = position.x;
				float new_positionz = position.z;
				if (w)
					{
						position.x -= forward.x *0.005;
						position.y -= forward.y *0.005;
						position.z -= forward.z *0.005;
					}
				if (alevel.check_wall(position.x, position.z) == false)
				{
					position.x = new_positionx;
					position.z = new_positionz;
				}
				if (s)
					{
						position.x += forward.x * 0.005;
				...
				if (d)
				{
					position.x -= side.x *0.005;
					position.y -= side.y * 0.005;
					position.z -= side.z *0.005;
				}
				if (alevel.check_wall(position.x, position.z) == false)
				{
					position.x = new_positionx;
